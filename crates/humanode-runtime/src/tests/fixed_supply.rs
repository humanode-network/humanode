...
/// A helper function to prepare basic native accounts with balances.
fn prepare_basic_native_accounts(
    endowed_accounts: Vec<AccountId>,
    pot_accounts: Vec<AccountId>,
...
/// A helper function to prepare basic evm accounts with balances.
fn prepare_basic_evm_accounts(
    endowed_accounts: Vec<EvmAccountId>,
    pot_accounts: Vec<EvmAccountId>,
...
fn new_test_ext_with() -> sp_io::TestExternalities {
    ...
    let basic_native_accounts = prepare_basic_native_accounts(endowed_accounts, pot_accounts);

    let evm_endowed_accounts = vec![evm_account_id("EvmAlice"), evm_account_id("EvmBob")];
    let basic_evm_accounts = prepare_basic_evm_accounts(evm_endowed_accounts, vec![]);
