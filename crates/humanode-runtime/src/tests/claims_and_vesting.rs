...
/// This test verifies that CheckTokenClaim signed extension validate works in the happy path.
#[test]
fn signed_extension_check_token_claim_works() {
    ...
    new_test_ext_with().execute_with(move || {
        ...
        let len = 0;
        assert_ok!(
            pallet_token_claims::CheckTokenClaim::<Runtime>::new().validate(
                &account_id("Alice"),
                &call,
                &normal,
                len
...
/// This test verifies that CheckTokenClaim signed extension validate fails if ethereum_address
...
/// This test verifies that CheckTokenClaim signed extension validate fails in case not existing claim.
#[test]
...
/// This test verifies that ChargeTransactionPayment signed extension works in the happy path without paying fee.
#[test]
