...
pub async fn new_full(config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    let (eth_debug_task, eth_debug_requester) = if ethereum_rpc_config
        .tracing_mode
    ...
    if let Some(eth_debug_task) = eth_debug_task {
        task_manager
            .spawn_essential_handle()
            .spawn("debug", Some("eth-tracing"), eth_debug_task);
    }

    let (eth_trace_task, eth_trace_requester) = if ethereum_rpc_config
        .tracing_mode
        .contains(&EthTracingMode::Trace)
    {
        ...
        (Some(trace_task), Some(trace_requester))
    } else {
        (None, None)
    };

    if let Some(eth_trace_task) = eth_trace_task {
        task_manager
            .spawn_essential_handle()
            .spawn("trace", Some("eth-tracing"), eth_trace_task);
    }
